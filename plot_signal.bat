...
if "%3"=="" (
	...
	set "term=svg font 'Arial,8' size 320,300 linewidth 0.5 background '#ffffff'; set pointsize 0.7; set output '%~3'"			
)
...
start /B gnuplot -persist -e "aether_data_file='%1'; aether_x_axis='%xaxis%';aether_y_axis='%yaxis%';aether_data_title='%data_title%';aether_theorie_title='%theorie_title%';aether_title='%title%';set decimalsign '%decimalsign%';set terminal %term%" signal.gnuplot
